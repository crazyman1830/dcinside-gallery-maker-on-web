...
export const generateWorldviewSpecificInstructions = (
  ...
  const worldviewLabelKoreanPart = selectedWorldview
    ? selectedWorldview.label.split(' (')[0]
    : worldviewValue;
  ...
  const selectedEra =
    WORLDVIEW_ERA_OPTIONS.find(era => era.value === currentEraValue) ||
    WORLDVIEW_ERA_OPTIONS.find(era => era.value === DEFAULT_WORLDVIEW_ERA)!;
  ...
  switch (currentEraValue) {
    case 'PREHISTORIC':
      ...
      eraConstraints =
        "STRICTLY PROHIBITED: Metal, Writing, Wheels, Farming, Architecture beyond caves/huts. REPLACEMENT: 'House'->'Cave', 'Weapon'->'Stone/Club', 'Clothing'->'Fur'.";
      break;
    case 'ANCIENT':
      eraDesc = 'Ancient Era (Bronze/Iron Age). Early Empires.';
      eraConstraints =
        "STRICTLY PROHIBITED: Gunpowder, Printing Press, Engines, Electricity, Plastic. REPLACEMENT: 'Car'->'Chariot/Horse', 'Paper'->'Scroll/Tablet'.";
      break;
    case 'MEDIEVAL':
      eraDesc = 'Medieval Era (Feudalism). Swords and cold weapons.';
      eraConstraints =
        "STRICTLY PROHIBITED: Guns/Rifles (unless primitive cannons), Steam Engines, Electricity, Plastic, Cars, Trucks, Smartphones, Internet. REPLACEMENT: 'Truck'->'Wagon/Cart', 'Car'->'Carriage', 'Phone'->'Letter/Magic Stone', 'Internet'->'Town Square'.";
      break;
    case 'EARLY_MODERN':
      eraDesc = 'Early Modern (19th - Early 20th Century). Steam & Steel, Industrial Revolution.';
      eraConstraints =
        "STRICTLY PROHIBITED: Digital Computers, Microchips, Internet, Smartphones, Nuclear Power. REPLACEMENT: 'Internet'->'Newspaper/Telegram', 'Smartphone'->'Pocket Watch/Notebook', 'Blog'->'Journal'.";
      break;
    case 'CONTEMPORARY':
...
`;
  ...
  return {
    ...
    selectedWorldview,
    worldviewLabelKoreanPart,
    eraConstraints,
  };
};

export const generateToxicitySpecificInstructions = (toxicityLevelValue: string) => {
  const selectedToxicity =
    TOXICITY_LEVEL_OPTIONS.find(tl => tl.value === toxicityLevelValue) ||
    TOXICITY_LEVEL_OPTIONS.find(tl => tl.value === DEFAULT_TOXICITY_LEVEL)!;
...
export const generateUserProfileInstructions = (
  ...
  if (Array.isArray(ageRangeValue) && ageRangeValue.length > 0) {
    const labels = AGE_RANGE_OPTIONS.filter(o => ageRangeValue.includes(o.value))
      .map(o => o.label)
      .join(', ');
...
export const getNicknameInstructionDetails = (anonymousNickRatioValue: string) => {
  const ratioOption =
    ANONYMOUS_NICK_RATIO_OPTIONS.find(opt => opt.value === anonymousNickRatioValue) ||
    ANONYMOUS_NICK_RATIO_OPTIONS.find(opt => opt.value === DEFAULT_ANONYMOUS_NICK_RATIO)!;
...
export const generatePlayerStatusInstructions = (userProfile?: UserProfile) => {
  ...
  const fullNickname =
    userProfile.nicknameType === 'ANONYMOUS' && userProfile.ip
  ...
  if (reputation <= 20) {
    statusDescription =
      '**PUBLIC ENEMY (VILLAIN)**. This user is hated. Generated comments MUST mock, insult, and attack this user. Do NOT agree with them.';
  } else if (reputation <= 40) {
    statusDescription =
      "**UNPOPULAR/ANNOYING**. This user is dismissed as a troll or idiot. React with sarcasm or 'nobody asked' attitude.";
  } else if (reputation <= 60) {
    statusDescription =
      '**NEUTRAL/INVISIBLE**. Treat as a random user. Standard reactions based on content quality.';
  } else if (reputation <= 80) {
    statusDescription =
      '**POPULAR/LIKED**. This user is well-regarded. React positively, agree with them, and defend them from critics.';
  } else {
    statusDescription =
      '**LEGEND/IDOL (GOD)**. This user is worshipped. Generated comments MUST blindly agree, praise, and treat their word as law.';
  }
