...
export const WorldviewSection: React.FC<WorldviewSectionProps> = ({
  ...
  return (
    <div className="space-y-6">
      <div>
        <label htmlFor="worldview" className={labelClass}>
          세계관 선택
        </label>
        <div className="relative">
          ...
          >
            {worldviewOptions.map(wv => (
              <option key={wv.value} value={wv.value}>
                {wv.label}
              </option>
            ))}
          </select>
      ...
      {selectedWorldview === 'CUSTOM' && (
        <div className="animate-fade-in">
          <div className="flex items-center">
            <label htmlFor="customWorldviewText" className={labelClass}>
              ...
              <span className="ml-1 text-xs font-normal text-slate-600">
                ({customWorldviewText.length}/{maxCustomWorldviewLength})
              </span>
            </label>
          ...
          <textarea
            ...
            aria-invalid={!!errors.customWorldviewText}
            aria-describedby={errors.customWorldviewText ? 'custom-worldview-error' : undefined}
          />
          {errors.customWorldviewText && (
            <p
              id="custom-worldview-error"
              role="alert"
      ...
      <div>
        <label htmlFor="worldviewEra" className={labelClass}>
          시간대 설정
        </label>
        <div className="relative">
          <select
        ...
        <p className="ml-1 mt-2 text-xs text-slate-600">
          ...
          선택한 세계관과 시간대를 조합하여 독창적인 설정을 만들 수 있습니다. (예: 현대 무협, 중세
          사이버펑크 등)
        </p>
      </div>
    </div>
