...
const outputsExist = async () => {
  try {
    await Promise.all(requiredOutputs.map(output => stat(output)));
    return true;
  } catch {
    return false;
  }
...
if (command === '--write') {
  ...
  await writeFile(statePath, `${JSON.stringify({ version: 1, hash }, null, 2)}\n`, 'utf8');
...
try {
  ...
  if (state.version === 1 && state.hash === hash && (await outputsExist())) {
